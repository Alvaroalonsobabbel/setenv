...
func setFlags() {
	...
	flag.Func("stagekey", "Sets the Key that will have the stage name. Allowed values: vault, item, vars", projectData.validateStageKey)
	flag.Func("stage", "Sets the stage of the project. Allowed values: test, staging, prod, aws", projectData.validateStage)
	flag.Func("addvar", "Adds one or more vars, comma separated. Colon separated for vars with key:value", projectData.addVar)
	flag.Func("rmvar", "Removes one or more vars, comma separated, only keys", projectData.rmVar)
